...
namespace Light.Json.PrimitiveParsing
{
    ...
    {
        public static IntegerParseResult TryParseInt32(this ReadOnlySpan<byte> source, out int value, out int bytesConsumed)
        {
            if (source.Length < 1)
            {
            ...
            var parsedValue = (uint) currentDigit;

            // Check if the source cannot overflow
            var overflowIndex = currentIndex + 10; // a 32 bit integer can consist of at most 10 digits
            if (source.Length < overflowIndex)
            {
                for (++currentIndex; currentIndex < source.Length; ++currentIndex)
                {
                    ...
                    parsedValue = parsedValue * 10 + (uint) currentDigit;
                }
            }
            else
            {
                ...
                for (currentIndex = overflowIndex - 1; currentIndex < source.Length; ++currentIndex)
                {
                    ...
                    if (parsedValue > int.MaxValue / 10 || parsedValue == int.MaxValue / 10 && isNextDigitTooLarge)
                    {
                        bytesConsumed = default;
                        value = default;
        ...
        public static IntegerParseResult TryParseInt32(this ReadOnlySpan<char> source, out int value, out int bytesConsumed)
        {
            if (source.Length < 1)
            {
                bytesConsumed = default;
            ...
            // Check if the source cannot overflow
            var overflowIndex = currentIndex + 10; // a 64 bit integer can consist of at most 19 digits
            if (source.Length < overflowIndex)
            {
                for (++currentIndex; currentIndex < source.Length; ++currentIndex)
                {
